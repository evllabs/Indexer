...
public class GradientIcon implements Icon {
	...
	/**
	 * Simple constructor that accepts a Color instance and uses it to draw the
	 * gradient.
	 * 
	 * @param width
	 *            The desired width of the icon
	...
	/**
	 * Complex constructor that accepts several additional parameters. Start and
	 * end intensity settings should be a float value from 0.0f to 1.0f, where a
	 * higher number results in a brighter white at the top of the gradient and
	 * a darker black at the bottom of the gradient. Smaller intensities will
	 * produce softer color transitions in the gradient.
	 * 
	 ...
	 *            The intensity of the white at the top of the gradient. A high
	 *            intensity will result in a harsher color change from the
	 *            primary color to the white.
	 * @param endIntensity
	 *            The intensity of the black at the bottom of the gradient. A
	 *            high intensity will result in a harsher color change from the
	 *            primary color to the black.
	 */
	public GradientIcon(int width, int height, int color, float startIntensity,
			float endIntensity) {
	...
	public void paintIcon(Component component, Graphics g, int x, int y) {
		...
		int gradientStart = (int) (y - height * startMultiplier), gradientSwitch = (int) (height * switchLocation), gradientEnd = (int) (y
				+ height + height * endMultiplier);

		GradientPaint p1 = new GradientPaint(x, gradientStart, new Color(
				0xffffff), x, y + gradientSwitch, color), p2 = new GradientPaint(
				x, y + gradientSwitch, color, x, gradientEnd, new Color(
						0x000000));
