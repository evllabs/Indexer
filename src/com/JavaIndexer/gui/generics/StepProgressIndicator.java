...
public class StepProgressIndicator extends JPanel {
	...
	public final Font font = new Font("Serif", Font.PLAIN, 16);
	public final Font boldFont = new Font("Serif", Font.ITALIC, 16);
	...
	public void paintComponent(Graphics g) {
		 ...
		 * order to properly orient the step names and lines. All of it will be
		 * vertically centered, and evenly distributed horizontally.
		 */
		...
		int horizSpacing = (width - sumNameWidths) / (stepNames.size() + 1);
		...
		// Calculate gradients and paint them for the line designating
		// incomplete tasks
		Color under_startColor = new Color(0xdf1f1f1);
		Color under_midColor = new Color(0xd4d4d4);
		Color under_endColor = new Color(0x6b6b6b);

		GradientPaint p1 = new GradientPaint(currentX, currentY,
				under_startColor, currentX, currentY + gradientSwitch,
				under_midColor), p2 = new GradientPaint(currentX, currentY
				+ gradientSwitch, under_midColor, currentX, currentY
				+ LINE_HEIGHT, under_endColor);
		...
		g2d.fillArc(endOfIncompleteLine - LINE_HEIGHT / 2, currentY,
				LINE_HEIGHT, LINE_HEIGHT, 90, -90);
		...
		g2d.setPaint(p2);
		g2d.fillRect(currentX, currentY + gradientSwitch, endOfIncompleteLine,
				LINE_HEIGHT - gradientSwitch);
		g2d.fillArc(endOfIncompleteLine - LINE_HEIGHT / 2, currentY,
				LINE_HEIGHT, LINE_HEIGHT, 0, -90);
		g2d.setColor(outlineColor);
		g2d.drawLine(currentX, currentY + LINE_HEIGHT, endOfIncompleteLine,
				currentY + LINE_HEIGHT);
		g2d.drawArc(endOfIncompleteLine - LINE_HEIGHT / 2, currentY,
				LINE_HEIGHT, LINE_HEIGHT, 90, -180);

		// Calculate gradients and paint them for the line designating complete
		// tasks
		Color over_startColor = new Color(0xd0d0ff);
		Color over_midColor = new Color(0x2b2bc6);
		Color over_endColor = new Color(0x2a2a4d);

		p1 = new GradientPaint(currentX, currentY - OVER_HEIGHT,
				over_startColor, currentX, currentY + gradientSwitch,
				over_midColor);
		p2 = new GradientPaint(currentX, currentY + gradientSwitch,
				over_midColor, currentX, currentY + LINE_HEIGHT + OVER_HEIGHT,
				over_endColor);

		// Draw the top part of the line designating complete tasks
		g2d.setPaint(p1);
		g2d.fillRect(currentX, currentY - OVER_HEIGHT, endOfCompleteLine,
				gradientSwitch + OVER_HEIGHT);

		g2d.fillArc(endOfCompleteLine - (LINE_HEIGHT + OVER_HEIGHT * 2) / 2,
				currentY - OVER_HEIGHT, LINE_HEIGHT, LINE_HEIGHT + OVER_HEIGHT
						* 2, 90, -90);

		g2d.setColor(outlineColor);
		g2d.drawLine(currentX, currentY - OVER_HEIGHT, endOfCompleteLine,
				currentY - OVER_HEIGHT);

		// Draw the bottom part of the line designating complete tasks
		g2d.setPaint(p2);
		g2d.fillRect(currentX, currentY + gradientSwitch, endOfCompleteLine,
				LINE_HEIGHT - gradientSwitch + OVER_HEIGHT);
		...
		g2d.setColor(outlineColor);
		g2d.drawArc(endOfCompleteLine - (LINE_HEIGHT + OVER_HEIGHT * 2) / 2,
				currentY - OVER_HEIGHT, LINE_HEIGHT, LINE_HEIGHT + OVER_HEIGHT
						* 2, 90, -180);
